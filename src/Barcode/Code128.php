...
class Code128 extends AbstractBarcode
{
    ...
    public function __construct(
        ...
        $this->init(UtfString::strcode2utf($code), $type, $ean);
     ...
     * @throws BarcodeException
     */
    protected function init(string $code, string $type, bool $ean)
    {
        switch (strtoupper($type)) {
            case self::TYPE_RAW:
                    ...
                    if (is_numeric($v) && round($v, 0) == $v) {
                        if ($v >= 0 && $v <= 105) {
                            ...
                            throw new BarcodeException(
                                sprintf(
                                    'Invalid CODE128RAW barcode value "%s". 0-105 needed',
                                    $code
                                )
                            );
                        }
                        ...
                        if ($v != '') {
                            throw new BarcodeException(
                                sprintf('Invalid CODE128RAW barcode value "%s". 0-105 needed', $code)
                            );
                        }
                    }
                }
                ...
                if ($startId < 103 || $startId > 105) {
                    throw new BarcodeException(
                        sprintf('Invalid CODE128RAW startid value "%s". Must be 103, 104 or 105 (for A, B or C)', $startId)
                    );
                }
            ...
            case self::TYPE_C:
                ...
                $newCode = '';
                $halfStringLength = (strlen($code) / 2);

                for ($i = 0; $i < $halfStringLength; ++$i) {
                    if ($code[2 * $i] < "0" || $code[2 * $i] > "9"
                        || $code[2 * $i + 1] < "0" || $code[2 * $i + 1] > "9") {
                        throw new BarcodeException(
                            sprintf('Invalid character "%s" in CODE128C barcode value "%s"', $code[$i], $code)
                        );
                    }
        ...
        for ($i = 0; $i < $stringLength; ++$i) {
            if ($ean && $i == 0) {
                ...
                if (strpos($keys, $code[$i]) === false) {
                    throw new BarcodeException(
                        sprintf(
                            'Invalid character "%s" in CODE128%s barcode value "%s"',
                            $code[$i],
                            $type,
                            $code
                        )
        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            if (($i == 0) || ($ean && $i == 1) | ($i > ($len - 4))) {
                $char_num = ord($code[$i]);
                $seq = self::CHARACTER_MAP[$char_num];
                ...
                throw new BarcodeException(
                    sprintf(
                        'Invalid character "%s" in CODE128C barcode value "%s"',
                        $code[$i],
                        $code
                    )
                );
